...
export default function SignupPage() {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        const res = await authAPI.firebaseRegister(idToken, form.name);
  ...
  return (
    <section className="flex items-center justify-center min-h-[80vh] bg-gray-50 px-4">
      <div className="w-full max-w-md bg-white p-8 rounded-xl shadow-lg">
        <h3 className="text-2xl font-bold text-blue-700 mb-6 text-center">Create Account</h3>
        ...
        <form onSubmit={handleEmailSignup}>
          ...
          <input type="email" name="email" placeholder="Email" value={form.email} onChange={handleChange}
            className="w-full mb-3 px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" required />
          <input type="password" name="password" placeholder="Password (6+ characters)" value={form.password} onChange={handleChange}
            className="w-full mb-3 px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" required minLength={6} />
